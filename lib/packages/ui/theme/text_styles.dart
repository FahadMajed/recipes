import 'package:flutter/material.dart';
import 'package:recipes/packages/ui/theme/colors.dart';

const double fourteen = 14;
const double twelve = 12;

const largeHeading = TextStyle(
  fontWeight: FontWeight.w600,
  fontSize: 40,
  color: textColor,
);
//title med
const appBar = TextStyle(
  color: Colors.black,
  fontWeight: FontWeight.w400,
  fontSize: 18,
);

//title large
const buttomButton = TextStyle(
  color: textColor,
  fontSize: 20,
  fontWeight: FontWeight.w600,
);

const textButton = TextStyle(
  color: secondaryColor,
  fontSize: fourteen,
  fontWeight: FontWeight.bold,
);

const filledBottonPrimary = TextStyle(
  fontSize: 17,
  color: Colors.white,
  fontWeight: FontWeight.w600,
);

const filledBottonAccent = TextStyle(
  fontSize: 17,
  color: textColor,
  fontWeight: FontWeight.w600,
);

const titleSmall = TextStyle(
  color: Colors.black,
  fontSize: fourteen,
  fontWeight: FontWeight.w500,
);
const titleMid = TextStyle(
  color: Colors.black,
  fontSize: 16,
  fontWeight: FontWeight.w500,
);
const titleLarge = TextStyle(
  color: secondaryColor,
  fontSize: 18,
  fontWeight: FontWeight.w500,
);

const titleSmallBold = TextStyle(
  color: Colors.black,
  fontSize: fourteen,
  fontWeight: FontWeight.w800,
);

const titleMidBold = TextStyle(
  color: Colors.black,
  fontSize: 16,
  fontWeight: FontWeight.w600,
);

const titleLargeBold = TextStyle(
  color: secondaryColor,
  fontSize: 18,
  fontWeight: FontWeight.w800,
);

const titleExtraLargeBold = TextStyle(
  color: textColor,
  fontSize: 32,
  fontWeight: FontWeight.w600,
);

const titleExtraLargeBlueBold = TextStyle(
  color: Colors.blue,
  fontSize: 22,
  fontWeight: FontWeight.w800,
);

const titleExtraLarge = TextStyle(
  color: Colors.black,
  fontSize: 22,
  fontWeight: FontWeight.w500,
);

const price = TextStyle(
  color: Colors.black,
  fontSize: 48,
  fontWeight: FontWeight.w700,
);

const descMedBlack = TextStyle(
  color: Colors.black,
  fontSize: fourteen,
  fontWeight: FontWeight.w500,
);

final descMed = TextStyle(
  color: Colors.grey[700],
  fontSize: fourteen,
  fontWeight: FontWeight.w400,
);

final descSmall = TextStyle(
  color: Colors.grey[700],
  fontSize: twelve,
  fontWeight: FontWeight.w400,
);

const descSmallBlack = TextStyle(
  color: Colors.black,
  fontSize: twelve,
  fontWeight: FontWeight.w400,
);

final descMedBold = TextStyle(
  color: Colors.grey[800],
  fontSize: fourteen,
  fontWeight: FontWeight.w700,
);

final descSmallBold = TextStyle(
  color: Colors.grey[800],
  fontSize: twelve,
  fontWeight: FontWeight.w600,
);

final iconLabelSmall = TextStyle(
  fontSize: twelve,
  fontWeight: FontWeight.w400,
  color: Colors.grey[800],
);

const iconLabelMid = TextStyle(
  fontSize: fourteen,
  fontWeight: FontWeight.w500,
  color: Colors.black,
);

const iconLabelSmallBold = TextStyle(
  fontSize: twelve,
  fontWeight: FontWeight.w600,
  color: darkBlue900,
);

const iconLabelMidBoldWhite = TextStyle(
  fontSize: fourteen,
  fontWeight: FontWeight.w600,
  color: Colors.white,
);

const subtitleSmall = TextStyle(
  fontSize: 10,
  fontWeight: FontWeight.w400,
  color: Colors.grey,
);
const subtitleMid = TextStyle(
  fontSize: 11,
  fontWeight: FontWeight.w400,
  color: Colors.grey,
);

const subtitleSmallBold = TextStyle(
  fontSize: 10,
  fontWeight: FontWeight.w600,
  color: Colors.grey,
);
const subtitleMidBold = TextStyle(
  fontSize: 11,
  fontWeight: FontWeight.w600,
  color: textColor,
);

const selectedText = TextStyle(
  color: Colors.white,
  fontSize: twelve,
  fontWeight: FontWeight.bold,
);
